...
d.ready(function () {
        ...
        $('#button-prev').on('click', function () {
            if ((status.includes('alive') || status.includes('dead') || status.includes('unknown')) && page > 1) {
                page--
                localStorage.setItem('page', page)
                $('article[class="characterCard"]').remove();
                getAllCharactersInfo()
        ...
        $('.button-register').on('click', function () {
            ...
            let password = /(?=^.{8,}$)((?=.*\d)|(?=.*\W+))(?![.\n])(?=.*[A-Z])(?=.*[a-z]).*$/;
            let succeed = '<form class="survey-form-success"><h1>Register succeded!</h1><div class="check-container"><img src="../resources/images/check.gif" alt="check-gift"/></div></form>'
            if ($('#name-register').val().length === 0 || !email.test($('#email-register').val()) || !password.test($('#password-register').val())) {
                return;
            }
            else {
        ...
        $('.button-login').on('click', function () {
            if (localStorage.getItem('email') === $('#email').val() && localStorage.getItem('password') === $('#password').val()) {
                ...
                if ($("#invalid").length === 0) $("#password").after('<p class="condition-password" id="invalid">Email o contraseña incorrecta</p>');
            }
        })
        ...
        function getAllCharactersInfo() {
            ...
            $.ajax({
                ...
                for (let i = 0; i < characterData.length; i++) {
                    ...
                    //let gridCharacter = '<div class="box" id="'+ characterId + '"><img src="' + characterImg + '" alt="' + characterName + '-img"><p>' + characterName + '</div>';              
                
                    let gridCharacter =
                        '<article class="characterCard">' +
                        '<div class="characterCard_img" id="' + characterId + '">' +
                        '<img src="' + characterImg + '" alt="' + characterName + '-img">' +
                        '</div>' +
                        '<div class="characterCard_ContentWrapper">' +
                        '<h2>' + characterName + '</h2>' +
                        '<span class="status__icon"></span>' +
                        '<span>Género: ' + characterGender + '</span>' +
                        '<span>Especie: ' + characterSpecies + '</span>' +
                        '<span class="text-gray">Visto por última vez: ' + characterLocation + '</span> ' +
                        '<span class="text-gray">Origen: ' + characterOrigin + '</span> ' +
                        '</div></article>';
...
function checkUrl(){
    if(( !('URL' in localStorage) || localStorage.getItem('URL') === '' || localStorage.getItem('URL') == undefined)){
        localStorage.setItem('URL' , "https://rickandmortyapi.com/api/character/?page=")
    }
    if(( !('page' in localStorage) || localStorage.getItem('page') === '' || localStorage.getItem('page') == undefined)){
        localStorage.setItem('page' , 1)
    }
    if(( !('status' in localStorage) || localStorage.getItem('status') === '' || localStorage.getItem('status') == undefined)){
        localStorage.setItem('status' , '&status=alive')
    }
}
//----------------------------------------------
